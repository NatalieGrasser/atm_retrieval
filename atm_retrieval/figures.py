...
def plot_pt(retr_obj,fs=12,figsize=5,comp_pt=True,show_cond=False,
            ...
            pm_temp=100,colors=[],show_legend=True,**kwargs):
    ...
    if retr_obj.target.name in ['2M0355','2M1425'] and comp_pt==True:
        file=np.loadtxt(f'{os.getcwd()}/models/PT_profiles/t1600g562nc_m0.0.dat')
        pres=file[:,1] # bar
        temp=file[:,2] # K
        ax.plot(temp,pres,linestyle='dashdot',c='blueviolet',linewidth=2)
        comparison_pt.append(Line2D([0], [0], color='blueviolet', linewidth=2, linestyle='dashdot',label='Sonora Bobcat \n$T=1600\,$K, log$\,g=4.75$'))
    if retr_obj.target.name=='test_ROXs12B':
        data = np.genfromtxt(f'{sonora_dir}/structures_m+0.0/t2200g100nc_m0.0.dat', skip_header=1)
        pres = data[:, 1]# bar
        temp = data[:, 2]
        ax.plot(temp,pres,linestyle='dashdot',c='r',linewidth=2)
        comparison_pt.append(Line2D([0], [0], color='r', linewidth=2, linestyle='dashdot',label=f'Sonora Bobcat \n$T=2200\,$K\nlog$\,g=4.0$\n[Fe/H]$=$0.0'))   
    if {'ROXs12B', 'ROXs12_onlyB'} & {retr_obj.target.name, retr_obj2.target.name} and comp_pt==True:
        comp_pt_temp = int(2200)
        met = -0.5 # +0.0
        if met<0:
            met = str(met)
        else:
            met = f'+{met}'
        data = np.genfromtxt(f'{sonora_dir}/structures_m{met}/t{comp_pt_temp}g100nc_m{met}.dat', skip_header=1)
        pres = data[:, 1]# bar
        temp = data[:, 2]
        ax.plot(temp,pres,linestyle='dashdot',c='b',linewidth=2)
        labl = f'Sonora Bobcat \n{teff}$={comp_pt_temp}\,$K\nlog$\,g=4.0$\n[Fe/H]$=${met}'
        comparison_pt.append(Line2D([0], [0], color='b', linewidth=2, linestyle='dashdot',label=labl))
    if 'ROXs12A' in [retr_obj.target.name,retr_obj2.target.name] and comp_pt==True:
        #file=np.loadtxt(f'{sphinx_pt_dir[:-1]}/Teff_3900.0_logg_4.25_logZ_-0.25_CtoO_0.5.txt')
        file=np.loadtxt(f'{sphinx_pt_dir[:-1]}/Teff_4000.0_logg_4.25_logZ_+0.0_CtoO_0.5.txt')
        temp=file[:,0] # bar
        pres=file[:,1] # K
        ax.plot(temp,pres,linestyle='dashdot',c='r',linewidth=2)
        #label = r"SPHINX model\n$T_{\mathrm{eff}} = 4000\,\mathrm{K}$\n$\log g = 4.25$\n[Fe/H] = 0.0"
        labl = f'SPHINX model \n{teff}$=4000\,$K\nlog$\,g=4.25$\n[Fe/H]$=0.0$'
        comparison_pt.append(Line2D([0], [0], color='r', linewidth=2, linestyle='dashdot',label=labl))
    ...
    if retr_obj.target.name in ['Sorg1X','Sorg20X']:
        ...
        ax.plot(tab.temperature,tab.pressure,linestyle='dashdot',c=retr_obj.target.in_col,linewidth=2)
        comparison_pt.append(Line2D([0], [0], color=retr_obj.target.in_col, linewidth=2, linestyle='dashdot',label='Input $P$-$T$'))
    ...
    def plot_temperature(retr_obj,ax,olabel,color): 
        if retr_obj.PT_type=='PTknot':
            ...
            for x in [1,2,3]: # plot 1-3 sigma errors
                ...
                ax.fill_betweenx(retr_obj.pressure,lower,upper,color=color,alpha=0.15)
            ax.scatter(medians,10**retr_obj.model_object.log_P_knots,color=color)
            temp_median = CubicSpline(log_P_knots,medians)(np.log10(retr_obj.pressure))
            ax.plot(temp_median,retr_obj.model_object.pressure,color=color,lw=2) 

            xmin=np.min(lower)-20#-100
            xmax=np.max(upper)+20#+100
            lines.append(Line2D([0],[0],marker='o',color=color,markerfacecolor=color,
            ...
            ax.plot(temperature,retr_obj.model_object.pressure,color=color,lw=2) 
            ...
            xmin=np.min((quantiles[:,0],quantiles[:,-1]))-pm_temp
            xmax=np.max((quantiles[:,0],quantiles[:,-1]))+pm_temp
    ...
    if retr_obj.chemistry in ['freechem','varchem']:
        ...
        if split_corner: # split corner plot to avoid massive files
            ...
            if retr_obj.chemistry=='quequchem':
                for val in ['log_Pqu_CO_CH4','log_Pqu_NH3','log_Pqu_HCN','log_Pqu_H2O_OH_O']:
                    if val in retr_obj.parameters.params:
...
def summary_plot(retr_obj,**kwargs):
    ...
    if retr_obj.instrument=='LIFE':
        ...
        ax_res.cla()
        ax_res.axis('off')
        ax_spec.set_xlabel('Wavelength [$\mathrm{\mu}$m]',fontsize=fs)
        ...
        if retr_obj.parameters.constant_params['fix_PT']==False:
        ...
        if retr_obj.parameters.params['fix_all_except_PT']==False:
        ...
        #opacity_plot(retr_obj,n=10,ax=ax_opa,fs=fs)
...
def opacity_plot(retr_obj,only_params=None,n=7,fs=10,
                 smallrange=False,addname=False,**kwargs): # n most abundant species

    only_params=[]
    abunds=[]
    pRT_names=[]
    labels=[]
    not_plot = ['H2','He','H-'] # not line species
    species = [x for x in retr_obj.species_names if x not in not_plot]
    
    if retr_obj.chemistry in ['freechem','varchem']:
        #suffix='_0' if retr_obj.chemistry=='varchem' else ''
        for spec in species:
            if spec in retr_obj.vary_species and spec not in not_plot:
                median_at_maxemcont = np.nanmedian(np.array(retr_obj.VMR_dict[spec])[:,retr_obj.params_dict['idx_maxcont']])
                abunds.append(median_at_maxemcont)
            #suffix='_0' if spec in retr_obj.vary_species else ''
            elif spec not in not_plot:
                #abunds.append(retr_obj.params_dict[f"log_{spec}{suffix}"])
                abunds.append(retr_obj.params_dict[f"log_{spec}"])
        
    elif retr_obj.chemistry in ['equchem','quequchem','flexequ']: # use VMRs where emission contribution is maximal
        for spec in species:
            if spec not in not_plot:
...
def compare_retrievals(retr_obj1,retr_obj2,fs=12,with_pt=True,
    ...
    #fig, titles1 = plot_corner(both_posteriors,retr_obj1,labels,fig,getfig=True,setfig=True)
...
def VMR_plot(retr_obj,fs=10,n=8,VMR_species=None,comp_equ=False,sigma=2,
    ...
    def plot_VMRs(retr_obj,ax,ax2,ls=None):
        ...
        alpha=1
        #if retr_obj.chemistry in 'freechem' and 'retr_obj2' not in kwargs:
...
def CCFs_compared(retr_obj, retr_obj2, ccf_species, figsize=None,
                  legend_labels=None, fs=12, save_jpg=False):

    nrows = len(ccf_species)
    ...
    # Ensure 2D axes if only one species
    if nrows == 1:
        axes = np.array([axes])

    for i, species in enumerate(ccf_species):

        # First column → retrieval 1
        CCF_plot_all(
            retr_obj,
            [species],
            axes=axes[i, 0],
    ...
    axes[2,0].set_ylim(-4,7.5)
    axes[3,1].set_xticks(axes[3,1].get_xticks()[1:-1])            
    fig.tight_layout()
    plt.subplots_adjust(wspace=0, hspace=0)
    fig.supxlabel(r'$v_{\rm rad}$ [km/s]', y=-0.04,fontsize=fs)
    fig.supylabel('S/N', x=-0.04,fontsize=fs)
    fig.savefig(f'{retr_obj.output_dir}/comparison/CCFs_both.pdf', bbox_inches='tight')
    if save_jpg:
        fig.savefig(f'{retr_obj.output_dir}/comparison/CCFs_both.jpg', bbox_inches='tight')
    plt.close()
...
# for CRIRES+ K-band
def plot_CO_orders(retr_obj,retr_obj2=None,order=5,save_jpg=False,
                    figsize=(7,3.5),rescale=True):

    crires_shape = (retr_obj.n_orders,retr_obj.n_dets,retr_obj.n_pixels)
    
    rets = [retr_obj]
    gs = [1] # panel/row 1
    if retr_obj2 is not None:
        rets.append(retr_obj2)
        gs.append(1) # another panel
    gs.append(0.3) # for residuals

    gridspec_kw={'height_ratios':gs}
    fig,ax=plt.subplots(len(rets)+1,1,figsize=figsize,dpi=200,
                        gridspec_kw=gridspec_kw,sharex=True)
    zorders = [3,2]

    for i,ret in enumerate(rets):

        if rescale:
            model = pRT_spectrum(ret)
            m_flux = model.make_spectrum()
            continuum = model.model_continuum.reshape(crires_shape)

        res=(ret.data_flux-ret.model_flux).reshape(crires_shape)
        to_reshape= [np.copy(ret.data_flux), ret.data_err, ret.data_wave, np.copy(ret.model_flux)]
        data_flux, data_err, data_wave, model_flux = [var.reshape(crires_shape) for var in to_reshape]
        s2 = ret.params_dict['s2'].reshape((crires_shape[:-1]))

        if rescale:
            scale = [data_flux, data_err, model_flux]
            data_flux, data_err, model_flux = [var*continuum for var in scale]

        if ret.target.name =='ROXs12_onlyB':
            obj_name='ROXs 12B'
        elif ret.target.name =='ROXs12A':
            obj_name='ROXs 12A'
        else:
            obj_name = ret.target.name
        label = f'Fit to {obj_name}'

        for det in range(3):
            ax[i].plot(data_wave[order,det],data_flux[order,det],lw=0.8,alpha=1,c='k')
            lbl = None if det!=0 else label
            ax[i].plot(data_wave[order,det],model_flux[order,det],lw=0.8,alpha=0.9,c=ret.color,label=lbl)
            ax[i].set_xlim(np.nanmin(data_wave[order])-1,np.nanmax(data_wave[order])+1)
            lower=data_flux[order,det]-data_err[order,det]*s2[order,det]
            upper=data_flux[order,det]+data_err[order,det]*s2[order,det]
            ax[i].fill_between(data_wave[order,det],lower,upper,color='k',alpha=0.2)
            err = data_err[order,det] if np.all(np.isnan(data_err[order,det]))==False else 0
            ax[i].set_ylabel('Flux')
            #if det==0:
                #ax[i].legend(loc='upper center')

            ax[-1].plot(data_wave[order,det],res[order,det],lw=0.8,alpha=1,c=ret.color,zorder=zorders[i])
            errmean=np.nanmean(err*s2[order,det])
            #ax[-1].errorbar(np.min(data_wave[order,det])-0.3, 0, yerr=errmean, 
                                #ecolor=ret.color, elinewidth=1, capsize=2)
    ...
    ncol = 2 if retr_obj2 is not None else 1
    fig.subplots_adjust(top=0.85)
    ax[0].legend(all_handles, all_labels, loc='upper center', ncol=ncol,
               bbox_to_anchor=(0.5, 1.5), frameon=False)
    fig.tight_layout()
    plt.subplots_adjust(wspace=0,hspace=0)
    fig.savefig(f'{retr_obj.output_dir}/comparison/CO_orders.pdf')
    if save_jpg:
        fig.savefig(f'{retr_obj.output_dir}/comparison/CO_orders.jpg')
    plt.close()

def plot_pt_vmrs(retr_obj,fs=12,VMR_species=None,xmin=1e-10,xmax=1e0,ymin=None,ymax=None,
                 legend_labels=None,comp_pt=True,temp_lim=None,show_contr=True,
                 contr_as_curve=False,leg_fs=9,save_jpg=False,**pt_kwargs):

    gridspec_kw={'width_ratios':[1,0.6]}
    fig,ax=plt.subplots(1,2,figsize=(7,4),dpi=200,
                        gridspec_kw=gridspec_kw)

    VMR_plot(retr_obj,ax=ax[0],VMR_species=VMR_species,fs=fs,
             wH2He=True,plotlegend=True,xmin=xmin,xmax=xmax)    
    plot_pt(retr_obj,ax=ax[1],fs=fs,legend_labels=legend_labels,comp_pt=comp_pt,
             temp_lim=temp_lim,show_contr=show_contr,
             contr_as_curve=contr_as_curve,leg_fs=leg_fs,**pt_kwargs)
    ...
    ax[1].xaxis.set_minor_locator(ticker.AutoMinorLocator(n=4))
    ax[1].set_ylabel(None)
    fig.tight_layout()
    plt.subplots_adjust(wspace=0,hspace=0)
    fig.savefig(f'{retr_obj.output_dir}/PT_VMRs.pdf')
    if save_jpg:
        fig.savefig(f'{retr_obj.output_dir}/PT_VMRs.jpg')
    plt.close()

def compare_pt_vmrs(retr_obj,retr_obj2,fs=12,VMR_species=None,xmin=1e-10,xmax=1e0,ymin=None,ymax=None,
                 legend_labels=[None,None],comp_pt=True,temp_lim=None,show_contr=True,
                 sigma=2,pm_temp=10,
                 contr_as_curve=False,leg_fs=9,colors=[],linestyles=[],**kwargs):

    output_dir = retr_obj.output_dir
    rets = [retr_obj,retr_obj2]
    if colors==[]:
        colors_list = [retr_obj.color,retr_obj2.color]
    else:
        colors_list = colors
    
    if 'retr_obj3' in kwargs:
        retr_obj3=kwargs.get('retr_obj3')
        rets.append(retr_obj3)
        if len(legend_labels)!=3:
            legend_labels.append(None)

    gridspec_kw={'width_ratios':[1,0.6]}
    fig,ax=plt.subplots(1,2,figsize=(6,3.5),dpi=200,gridspec_kw=gridspec_kw)

    #VMR_plot(retr_obj,ax=ax[0],VMR_species=VMR_species,fs=fs,linestyles=linestyles,
    #        show_contr=False,
     #        wH2He=True,plotlegend=True,xmin=xmin,xmax=xmax,retr_obj2=retr_obj2,retr_obj3=retr_obj3)    
    #plot_pt(retr_obj,retr_obj2=retr_obj2,retr_obj3=retr_obj3,ax=ax[1],fs=fs,legend_labels=legend_labels,comp_pt=comp_pt,
    #         temp_lim=temp_lim,show_contr=show_contr,colors=colors_list,
    #         contr_as_curve=contr_as_curve,leg_fs=leg_fs)

    # PT
    xmins,xmaxs=[],[]
    alpha=1 
    legend_lines = []
    for c,l,ls in zip(colors_list,legend_labels,linestyles):
        legend_lines.append(Line2D([0], [0], color=c, linewidth=2, linestyle=ls,label=l))
    for i,retr_obj in enumerate(rets):
        dlnT_dlnP_knots=[]
        derr=[]
        n_grad = sum(1 for key in retr_obj.params_dict if re.fullmatch(r'dlnT_dlnP_\d+', key))
        for x in range(n_grad):
            key=f'dlnT_dlnP_{x}'
            dlnT_dlnP_knots.append(retr_obj.params_dict[key]) # gradient median values
            derr.append(retr_obj.params_dict[f'{key}_err']) # -/+ errors
        derr=np.array(derr) # gradient errors
        ...
        # get 1-2-3 sigma of temp_dist, has shape (samples, n_atm_layers)
        quantiles = np.array([np.percentile(retr_obj.temp_dist[:,i], [0.2,2.3,15.9,50.0,84.1,97.7,99.8], axis=-1) for i in range(retr_obj.temp_dist.shape[1])])
        sm = quantiles[:, 3 - sigma]
        sp = quantiles[:, 3 + sigma]
        ax[1].fill_betweenx(retr_obj.pressure,sm,sp,color=colors_list[i],alpha=0.1)
        sigma_eff = 3 if sigma == 'all' else sigma
        xmin_=np.min(quantiles[:, 3 - sigma_eff])-pm_temp
        xmax_=np.max(quantiles[:, 3 + sigma_eff])+pm_temp
        xmins.append(xmin_)
        xmaxs.append(xmax_)
        ax[1].plot(temperature,retr_obj.pressure,color=colors_list[i],lw=2)  
        
        tab = PSG_input(retr_obj.target.name)
        ax[1].plot(tab.temperature,tab.pressure,linestyle='dashdot',c=retr_obj.target.in_col,linewidth=2)
        ...
        for species in VMR_species:
            ...
            if retr_obj.chemistry=='freechem':
                VMR=10**retr_obj.params_dict[f'log_{species}']
                p = 10**np.array(np.percentile(retr_obj.posterior[f'log_{species}'][0],[0.2,2.3,15.9,50.0,84.1,97.7,99.8], axis=-1))
                median = np.ones_like(retr_obj.pressure)*p[3]
                sm = np.ones_like(retr_obj.pressure)*p[3 - sigma]
                sp = np.ones_like(retr_obj.pressure)*p[3 + sigma]
            elif (retr_obj.chemistry in ['equchem','quequchem','flexequ','varchem']): 
                p=np.percentile(retr_obj.VMR_dict[species], [0.2,2.3,15.9,50.0,84.1,97.7,99.8], axis=0)
                median = p[3]
                sm = p[3 - sigma]
                sp = p[3 + sigma]
            ax[0].plot(median,retr_obj.pressure,label=label,alpha=alpha,linestyle=linestyle,c=color)
            ax[0].fill_betweenx(retr_obj.pressure,sm,sp,color=color,alpha=0.1)
    ...
    for i,retr_obj in enumerate(rets): 
        summed_contr =retr_obj.summed_emcont
        contribution_plot=summed_contr/np.max(summed_contr)*(xma-xmi)+xmi
        ax[1].plot(contribution_plot,retr_obj.pressure,linestyle='dotted',
                lw=1,alpha=0.8,color=colors_list[i]) 
        ax[0].axhline(y=10**retr_obj.params_dict['log_P_maxcont'], xmin=0, xmax=1,
                    alpha=contr_a, color=colors_list[i],linestyle='dashdot',linewidth=contr_lw)
        ax[1].axhline(y=10**retr_obj.params_dict['log_P_maxcont'], xmin=0, xmax=1,
                    alpha=contr_a, color=colors_list[i],linestyle='dashdot',linewidth=contr_lw)

    pres = PSG_input(retr_obj.target.name).pressure
    input_contr = np.load(f'LIFE/{retr_obj.target.name}/input_pRT_summed_contr.npy')
    input_contr =input_contr/np.max(input_contr)*(xma-xmi)+xmi
    idx_maxcont=np.where(input_contr == np.max(input_contr))[0][0]
    input_P_maxcont = pres[idx_maxcont] # pressure at max emission contribution
    ax[1].plot(input_contr,pres,linestyle='dotted',
                lw=1,alpha=0.8,color=retr_obj.target.in_col) 
    ax[1].axhline(y=input_P_maxcont, xmin=0, xmax=1,alpha=contr_a, color=retr_obj.target.in_col,linestyle='dashdot',linewidth=contr_lw)
    ax[0].axhline(y=input_P_maxcont, xmin=0, xmax=1,alpha=contr_a, color=retr_obj.target.in_col,linestyle='dashdot',linewidth=contr_lw)
    ...
    for species in VMR_species:
        ...
        if species=='H2O':
            #if retr_obj.target.const_H2O=='':
            ax[0].plot(tab[species],pres,alpha=1,linestyle='solid',c=retr_obj.target.in_col)
            #else:
            h2o_vmr = np.ones_like(tab[species])*0.023412
            ax[0].plot(h2o_vmr,pres,alpha=1,linestyle='dashdot',c=retr_obj.target.in_col)
        else:
            ax[0].plot(tab[species],pres,alpha=1,linestyle='dashdot',c=retr_obj.target.in_col)
    #chemleg.append(Line2D([0], [0], color='k',linestyle='solid',linewidth=2,alpha=0.7,label='Input'))
    #chemleg.append(Line2D([0], [0], color=retr_obj.color, linewidth=2,alpha=0.3, linestyle='dashdot',label='Retr $P_\mathrm{max}$'))
    #chemleg.append(Line2D([0], [0], color=retr_obj.target.in_col, linewidth=2,alpha=0.3, linestyle='dashdot',label='Input $P_\mathrm{max}$'))

    ax[0].set_ylim(min(retr_obj.pressure),max(retr_obj.pressure))
    ax[0].invert_yaxis()
    ax[0].set_yscale('log')
    ax[0].set_xscale('log')
    ...
    ax[1].set_xlim(xmi,xma)
    ax[1].set_ylim(min(retr_obj.pressure),max(retr_obj.pressure))
    ax[1].invert_yaxis()
    ax[1].set_yscale('log')
    ax[1].set_xlabel('Temperature [K]')
    legend_lines.append(Line2D([0], [0], color=retr_obj.target.in_col, linewidth=2, linestyle='solid',label=r'Input (var$_{\rm in}$)'))
    legend_lines.append(Line2D([0], [0], color=retr_obj.target.in_col, linewidth=2, linestyle='dashdot',label=r'Input (const$_{\rm in}$)'))
    ax[0].legend(handles=legend_lines,loc='upper center',handlelength=2.2,fontsize=fs*0.7)
    if ymin!=None and ymax!=None:
        ax[0].set_ylim(ymax,ymin)
        ax[1].set_ylim(ymax,ymin)
    ax[1].yaxis.set_major_locator(plt.NullLocator())
    ...
    fig.tight_layout()
    plt.subplots_adjust(wspace=0,hspace=0)
    fig.savefig(f'{output_dir}/comparison/PT_VMRs.pdf')
    plt.close()

def compare_pt_profiles(retr_obj,retr_obj2,fs=12,
                 legend_labels=[None,None],comp_pt=True,temp_lim=[None,None],
                 show_contr=True,contr_as_curve=False,
                 leg_fs=9,save_jpg=False,**kwargs):
    ...
    fig,ax=plt.subplots(1,len(rets),figsize=(6,4),dpi=200) 
    for i,ret in enumerate(rets):
        axi = ax[i]
        plot_pt(ret,ax=axi,fs=fs,legend_labels=legend_labels[i],
                comp_pt=comp_pt,temp_lim=temp_lim[0],show_contr=show_contr,
                contr_as_curve=contr_as_curve,leg_fs=leg_fs)
        ...
        if i>0:
            ...
            ax[i].xaxis.set_minor_locator(ticker.AutoMinorLocator(n=4))
            ax[i].set_ylabel(None)

    ax[0].xaxis.set_minor_locator(ticker.AutoMinorLocator(n=4))

    

    fig.tight_layout()
    plt.subplots_adjust(wspace=0,hspace=0)
    fig.savefig(f'{retr_obj.output_dir}/comparison/PT_profiles.pdf')
    if save_jpg:
        fig.savefig(f'{retr_obj.output_dir}/comparison/PT_profiles.jpg')
    plt.close()

def plot_ccf_residuals(retr_obj,residuals_list,species_template_list,species):

    fig,ax=plt.subplots(21,1,figsize=(5,30),dpi=200)
    wave = retr_obj.data_wave
    for i in range(len(residuals_list)):
        mask = retr_obj.mask_isfinite[i]
        ax[i].plot(wave[i][mask],residuals_list[i],label=f'Data - model w/o {species}',lw=0.9,c='k')
        ax[i].plot(wave[i][mask],species_template_list[i],label=f'Full model -model w/o {species}',lw=0.9,c='tab:orange')
    ax[0].legend()
    fig.tight_layout()
    #plt.subplots_adjust(wspace=0,hspace=0)
    ccf_dir = pathlib.Path(f'{retr_obj.output_dir}/CCF/')
    ccf_dir.mkdir(parents=True, exist_ok=True)
    fig.savefig(f'{retr_obj.output_dir}/CCF/ccf_res_{species}.pdf')
    plt.close()

def emission_contribution_animation(retr_obj, fps=5, save=True):
    wl = retr_obj.data_wave[0]
    fl = retr_obj.data_flux[0]
    err = retr_obj.data_err[0]

    model_object = pRT_spectrum(retr_obj, contribution=True)
    model_flux = model_object.make_spectrum(save_whole_contribution=True)
    contribution = model_object.emission_contribution  
    wave_um = model_object.wave_um
    summed_contr = model_object.summed_emcont

    f = interp1d(wave_um, contribution, axis=1, kind='linear', fill_value="extrapolate")
    contribution_interp = f(wl)    # shape (layers, n_data)
    n_frames = len(wl)

    fig, ax = plt.subplots(2, 2, figsize=(7,4), dpi=150,
                           gridspec_kw={'width_ratios':[0.6, 2],
                                        'height_ratios':[0.6,1]})
    xmin, xmax = plot_pt(retr_obj, fs=10, leg_fs=7, ax=ax[1,0],
                         contr_as_curve=True, get_xmin_xmax=True)

    ax[0,0].axis('off')
    ax[1,1].errorbar(wl, fl, yerr=err, fmt='o', markersize=2.5, elinewidth=0.9,
                   c='k', label='Data')
    ax[1,1].set_xlabel('Wavelength [um]')
    ax[1,1].set_ylim(0, np.nanmax(fl)*1.1)
    ax[1,1].yaxis.tick_right()
    ax[1,1].yaxis.set_label_position("right")
    ax[1,1].set_ylabel('Flux')

    colors, labels=[],[]
    line_props = opacity_plot(retr_obj,n=10,ax=ax[0,1],fs=7)
    for col,lab in line_props:
        colors.append(col)
        labels.append(lab)
    lines=[]
    ...
    legend=ax[0,1].legend(handles=lines,ncol=3,loc='upper center',fontsize=6)

    maxcont = np.max(contribution_interp)

    initial_plot_contr = contribution_interp[:, 0] / maxcont * (xmax - xmin) + xmin
    contr_line, = ax[1,0].plot(initial_plot_contr, retr_obj.pressure,
                             linestyle='dashed', lw=1.5, alpha=0.8, color='r')

    vline = ax[1,1].axvline(wl[0], linestyle='dashed', lw=1.5, alpha=0.8, color='r')
    #vline2 = ax[0,1].axvline(wl[0], linestyle='dashed', lw=1.5, alpha=0.8, color='r')
    ...
    def update(i):
        # update left curve
        plot_contr = contribution_interp[:, i] / maxcont * (xmax - xmin) + xmin
        contr_line.set_xdata(plot_contr)
        # update right vertical line
        vline.set_xdata([wl[i], wl[i]])
        #vline2.set_xdata([wl[i], wl[i]])
        return contr_line, vline

    anim = FuncAnimation(fig, update, frames=n_frames, 
                         interval=1000/fps, blit=True)

    if save:
        outfile = f"{retr_obj.output_dir}/emission_contribution.gif"
...
# compare vmrs to other ratios in equchem
def VMRs_equ_other_ratio(retr_obj,fs=10,n=8,VMR_species=None,comp_equ=False,
                plotlegend=False,wH2He=True,show_contr=True,
                xmin=1e-10,xmax=1e0,save_jpg=False,add_species=None,
                ymin=None,ymax=None,C_to_O=None,FeH=None,**kwargs):

    suffix='' if add_species==None else f'_{add_species}'
    if C_to_O is not None:
        suffix += f'_CtO{C_to_O}'
    if FeH is not None:
        suffix += f'_FeH{C_to_O}'
    fig,ax=plt.subplots(1,2,figsize=(5,3),dpi=200,
                        gridspec_kw={'width_ratios':[1,2]})
    line_props = VMR_plot(retr_obj,fs=10,n=8,VMR_species=VMR_species,comp_equ=comp_equ,
                plotlegend=plotlegend,wH2He=wH2He,show_contr=show_contr,
                xmin=xmin,xmax=xmax,add_species=add_species,
                ymin=ymin,ymax=ymax,ax=ax[1],**kwargs)  

    from retrieval import Retrieval
    from parameters import Parameters
    co = np.round(retr_obj.parameters.params['C/O'],decimals=2)
    params_comp = retr_obj.params_dict.copy()
    C_to_O = retr_obj.params_dict['C/O'] if C_to_O is None else C_to_O
    FeH = retr_obj.params_dict['Fe/H'] if FeH is None else FeH
    retr_obj.model_object=pRT_spectrum(retr_obj,add_species=add_species)
    params_comp.update({'C/O': C_to_O,'Fe/H': FeH})
    retr_obj.parameters.params = params_comp
    model_object=pRT_spectrum(retr_obj,add_species=add_species)

    alpha=0.9
    linestyle = 'dotted'
    VMR_species = VMR_species + list(add_species)
    for species_i in VMR_species:
        color=retr_obj.species_info.loc[species_i,'color']
        label=retr_obj.species_info.loc[species_i,'mathtext_name']
        ax[1].plot(model_object.VMR_dict[species_i],retr_obj.pressure,label=label,
                alpha=alpha,linestyle=linestyle,c=color)
    ...
    for col,lab in zip(colors,labels):
        lines.append(Line2D([0],[0],color=col,linewidth=2,label=lab))
    ax[1].legend(handles=lines,fontsize=fs*0.7)

    # plot sum of O bearing species over pressure
    vmr_o = np.zeros(retr_obj.pressure.shape)
    vmr_o_in = np.zeros(retr_obj.pressure.shape)

    for species_i in VMR_species:
        vmr_i = retr_obj.model_object.VMR_dict[species_i]
        vmr_o += vmr_i
        vmr_i_in = model_object.VMR_dict[species_i]
        vmr_o_in +=vmr_i_in
    vmr_o_frac = retr_obj.model_object.VMR_dict['O']/vmr_o
    vmr_o_in_frac = model_object.VMR_dict['O']/vmr_o_in
    
    
    co_in = np.round(C_to_O,decimals=2)
    ax[0].plot(vmr_o_frac,retr_obj.pressure,color='k',label=f'C/O={co}')
    ax[0].plot(vmr_o_in_frac,retr_obj.pressure,linestyle='dotted',color='k',label=f'C/O={co_in}')
    #ax[0].set_xscale('log')
    ax[0].set_yscale('log')
    ax[0].set_ylim(ymax,ymin)
    ax[0].set_ylabel('Pressure [bar]')
    ax[0].legend(fontsize=fs*0.7)
    ax[0].set_title(r'% of O in O')
    ax[1].set_yticks([])
    ax[1].set_ylabel('')

    fig.tight_layout()
    plt.subplots_adjust(wspace=0, hspace=0)
    fig.savefig(f'{retr_obj.output_dir}/VMRs{suffix}.pdf')
    if save_jpg:
        fig.savefig(f'{retr_obj.output_dir}/VMRs{suffix}.jpg')
    plt.close()
...
def plot_contribution_per_species(retr_obj,fs=10,plot_legend=True,**kwargs):
    atmosphere = retr_obj.atmosphere_objects
    species_names = retr_obj.species_names
    ...
    if retr_obj.parameters.params['fix_all_except_PT'] and 'Sorg' in retr_obj.target.name:
        tab = PSG_input(retr_obj.target.name).table
        all_species = retr_obj.species_names.copy()
        all_species.extend(['H2','He'])
        VMRs = {}
        for species_i in all_species:
            VMRs[species_i] = tab[species_i].values[::2]
...
# compare PT and blackbody for retrievals of Sorg where all fixed except PT
def compare_pt_bb(retr_obj,retr_obj2,fs=12,sigma=2,
                 legend_labels=[None,None],comp_pt=True,temp_lim=[None,None],
                 show_contr=True,contr_as_curve=False,
                 leg_fs=9,save_jpg=False,**kwargs):

    radtrans = retr_obj.atmosphere_objects
    gravity = 10**retr_obj.params_dict['log_g']
    ...
    if 'retr_obj4' in kwargs:
        ...
        rets.append(retr_obj4)
        if len(legend_labels)!=4:
            legend_labels.append(None)
    if 'retr_obj5' in kwargs:
        retr_obj5=kwargs.get('retr_obj5')
        rets.append(retr_obj5)
        if len(legend_labels)!=5:
            legend_labels.append(None)

    fig,axes=plt.subplots(1,2,figsize=(6,3.5),dpi=200,gridspec_kw={'width_ratios':[1,1]}) 
    ...
    def get_bb(ret,T=None,get_full_spec=True,wl_um = None):

        if T is None:
            temperatures = ret.model_object.temperature
        else:
            temperatures = T
        mass_fractions = ret.model_object.mass_fractions
        MMW = ret.model_object.MMW
        ...
        return wl_um, base_spec, blackbody_flux
        
    tab = PSG_input(retr_obj.target.name)
    T = np.interp(retr_obj.pressure, tab.pressure, tab.temperature)
    wl_um, _, bb_in = get_bb(retr_obj,T=T)
    ax_bb.set_xlim(min(wl_um),max(wl_um))

    ax_pt.invert_yaxis()
    ax_pt.set_yscale('log')
    ax_pt.plot(tab.temperature,tab.pressure,linestyle='dashdot',
                c=retr_obj.target.in_col,linewidth=2,label='Input')

    xmins, xmaxs = [], []
    colors = ['tab:blue','tab:orange','tab:green','tab:red']
    for i,ret in enumerate(rets):

        n_grad = sum(1 for key in ret.params_dict if re.fullmatch(r'dlnT_dlnP_\d+', key))

        wl_um, _, bb = get_bb(ret,get_full_spec=False,wl_um=wl_um)
        ax_bb.plot(wl_um,bb,color=colors[i],label=f'{n_grad} nodes')

        # --- continuum uncertainty envelope with caching ---
        cache_file = pathlib.Path(ret.output_dir) / "continuum_flux_samples.npy"

        if cache_file.exists():
            continuum_samples = np.load(cache_file)

        else:
            continuum_samples = []
            for temp_sample in ret.temp_dist:
                #print('temp_sample.shape',temp_sample.shape)
                _, _, bb_continuum = get_bb(retr_obj,T=temp_sample,
                                            get_full_spec=False,wl_um=wl_um)
                continuum_samples.append(bb_continuum)
            continuum_samples = np.array(continuum_samples)
            np.save(cache_file, continuum_samples)

        if sigma == 'all':
            perc = [0.2, 2.3, 15.9, 50.0, 84.1, 97.7, 99.8]
            q = np.percentile(continuum_samples, perc, axis=0)
            #ax_bb.plot(wl_um, q[3], color=colors[i])  # ← median
            ax_bb.fill_between(wl_um, q[0], q[-1], color=colors[i], alpha=0.08)
            ax_bb.fill_between(wl_um, q[1], q[-2], color=colors[i], alpha=0.10)
            ax_bb.fill_between(wl_um, q[2], q[-3], color=colors[i], alpha=0.15)

        else:
            sigma_to_percentile = {1: (15.9, 84.1),
                                    2: (2.3, 97.7),
                                        3: (0.2, 99.8)}
            p_low, p_high = sigma_to_percentile[sigma]
            lower = np.percentile(continuum_samples, p_low, axis=0)
            upper = np.percentile(continuum_samples, p_high, axis=0)
            ax_bb.fill_between(wl_um, lower, upper,
                            color=colors[i], alpha=0.15)
            #median = np.median(continuum_samples, axis=0)
            #ax_bb.plot(wl_um, median, color='k',linestyle='dashed') # just to check

        dlnT_dlnP_knots=[]
        derr=[]
        for x in range(n_grad):
            key=f'dlnT_dlnP_{x}'
            dlnT_dlnP_knots.append(ret.params_dict[key]) # gradient median values
            derr.append(ret.params_dict[f'{key}_err']) # -/+ errors
        derr=np.array(derr) # gradient errors
        T0=ret.params_dict['T0']
        err=ret.params_dict['T0_err']
        temperature=ret.model_object.make_pt(dlnT_dlnP_knots=dlnT_dlnP_knots,T_base=T0)
        ax_pt.plot(temperature,ret.model_object.pressure,color=colors[i],lw=2,label=f'{n_grad} nodes') 
        # get 1-2-3 sigma of temp_dist, has shape (samples, n_atm_layers)
        quantiles = np.array([np.percentile(ret.temp_dist[:,i], [0.2,2.3,15.9,50.0,84.1,97.7,99.8], axis=-1) for i in range(retr_obj.temp_dist.shape[1])])
        if sigma=='all':
            ax_pt.fill_betweenx(ret.pressure,quantiles[:,0],quantiles[:,-1],color=colors[i],alpha=0.15)
            ax_pt.fill_betweenx(ret.pressure,quantiles[:,1],quantiles[:,-2],color=colors[i],alpha=0.15)
            ax_pt.fill_betweenx(ret.pressure,quantiles[:,2],quantiles[:,-3],color=colors[i],alpha=0.15)
        else:
            sm = quantiles[:, 3 - sigma]
            sp = quantiles[:, 3 + sigma]
            ax_pt.fill_betweenx(ret.pressure,sm,sp,color=colors[i],alpha=0.1)
        sigma_eff = 3 if sigma == 'all' else sigma
        xmins.append(np.min(quantiles[:, 3 - sigma_eff])-10)
        xmaxs.append(np.max(quantiles[:, 3 + sigma_eff])+10)
    ...
    for i,ret in enumerate(rets):
        if contr_as_curve:
            summed_contr =ret.summed_emcont
            contribution_plot2=summed_contr/np.max(summed_contr)*(xmax-xmin)+xmin
            ax_pt.plot(contribution_plot2,ret.pressure,linestyle=contr_ls,
                    lw=contr_lw,alpha=contr_a,color=colors[i])
            #lines.append(Line2D([0], [0], color=retr_obj2.color, alpha=0.8,linewidth=1.5, 
            #                    linestyle='--',label=f'{retr_obj2.target.name} contr.'))
        else:
            photosphere_gradient(summed_contr,ret.pressure,ax_pt,colors[i],xmin,xmax)

    ax_bb.plot(wl_um,bb_in,color=retr_obj.target.in_col,
                linestyle='dashdot',label='Input')
    input_contr = np.load(f'LIFE/{retr_obj.target.name}/input_pRT_summed_contr.npy')
    prs = PSG_input(retr_obj.target.name).pressure
    if contr_as_curve:
        input_contr =input_contr/np.max(input_contr)*(xmax-xmin)+xmin
        ax_pt.plot(input_contr,prs,linestyle=contr_ls,
                lw=contr_lw,alpha=contr_a,color=retr_obj.target.in_col)
    else:
        photosphere_gradient(input_contr,prs,ax_pt,retr_obj.target.in_col,xmin,xmax)        

    #ax_bb.set_title('Continuum emission')
    #ax_pt.set_title('$P-T$ profiles')
    ax_pt.set_xlim(xmin,xmax)
    ax_pt.set_xlabel('Temperature [K]')
    ax_pt.set_ylabel('Pressure [bar]')
    ax_pt.set_ylim(1e0,1e-6)
    ax_bb.legend(fontsize=8)
    ax_bb.set_xlabel('Wavelength [μm]')
    ax_bb.set_ylabel("Flux [W/sr/m$^2$/μm]")

    #ax_bb.yaxis.set_major_locator(plt.NullLocator())
    #ax_bb.yaxis.set_minor_locator(plt.NullLocator())
    ax_pt.xaxis.set_minor_locator(ticker.AutoMinorLocator(n=5))
    ax_bb.xaxis.set_minor_locator(ticker.AutoMinorLocator(n=5))

    fig.tight_layout()
    fig_dir = pathlib.Path(f'{retr_obj.output_dir}/comparison')
    fig_dir.mkdir(parents=True, exist_ok=True)
    fig.savefig(f'{fig_dir}/PT_BB.pdf')
    if save_jpg:
        fig.savefig(f'{fig_dir}/PT_BB.jpg')
    plt.close()